...
const containsAnd = (arr) => {
  let ando = arr.filter(name=>name.includes('and'));
  return ando;};
...
const oddValues = (arr) => {
  let odd = arr.filter(number=>number%2);
  return odd;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArray = arr.filter((word) => !forbiddenValues.includes(word));
  return newArray;
};
